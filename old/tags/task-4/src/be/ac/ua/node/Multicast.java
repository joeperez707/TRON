...
public class Multicast implements Runnable{
	...
	public void assignId(){
		if (nodes.size() >= 2){
			...
			prevId = nextId;
			if (nextId > (Integer.MAX_VALUE - nextId)){
				setId(new Random(System.currentTimeMillis()).nextInt(nextId));
			}
			else {
				setId(new Random(System.currentTimeMillis()).nextInt(Integer.MAX_VALUE - nextId) + nextId);
			}
		} else {
			setId(new Random(System.currentTimeMillis()).nextInt(Integer.MAX_VALUE));
		}
	}
